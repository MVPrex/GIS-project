...
# =========================================================
st.set_page_config(
    page_title="Interactive Geospatial Analysis",
    page_icon="🌍",
    layout="wide",
...
st.markdown(
    ...
    <div style="
        ...
        margin-bottom:12px;
    ">
        <div style="font-size:clamp(16px,2vw,22px); font-weight:700; color:#eaf3fa; display:flex; align-items:center; gap:10px;">
            <span>🌍</span>
            <span>Interactive Geospatial Analysis</span>
            <span style="font-size:11px; font-weight:600; background:#122a3d; color:#4ea8de; padding:3px 8px; border-radius:12px; border:1px solid #1f4a6e;">
                2D & 3D Swipe Comparison
            </span>
        </div>
        <div style="font-size:12px; color:#7892a5;">
            GeoTIFF • GeoJSON • Multi-Temporal
        </div>
    </div>
    """,
    unsafe_allow_html=True
...
# LAYOUT COLUMNS
# =========================================================
left_col, center_col, right_col = st.columns([1.3, 5.4, 1.3], gap="small")
...
with left_col:
    ...
    if AUTO_LOAD_FOLDER:
        if DATA_FOLDER.is_dir():
            st.caption(f"Auto-loading: `{DATA_FOLDER}`")
        else:
            st.caption(f"Auto-load folder not found: `{DATA_FOLDER}`")
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Upload multi-temporal GeoTIFFs or GeoJSON boundary polygons"
    )

    c1, c2 = st.columns([1, 1])
    with c1:
        if st.button("🌟 Demo Data", help="Load synthetic multi-year satellite rasters & AOI polygon"):
    ...
    show_rasters = st.checkbox("Show Raster Layers", value=True)
    show_vectors = st.checkbox("Show GeoJSON Layers", value=True)

    raster_opacity = st.slider("Raster Opacity", min_value=10, max_value=100, value=92, step=2) / 100.0

    colormap_name = st.selectbox(
        "Colormap (Single-band)",
        options=list(COLORMAPS.keys()),
        index=0,
        help="Palette applied to 1-band rasters (DEM, NDVI, index)"
    )
...
with left_col:
    ...
    if not rasters and not vectors:
        st.caption("Load data to choose individual layers.")
    else:
...
with right_col:
    ...
    if not st.session_state["comparison_open"]:
        st.caption("Use the button below to open the layer comparison viewer.")
        if st.button(
            "⚖️ Compare Selected Layers",
        ...
        if len(active_rasters) < 2:
            st.info("Select at least two visible raster layers to compare.")
        ...
        st.success("Comparison viewer is open.")
    ...
    if st.session_state["comparison_open"] and len(active_rasters) >= 2:
        ...
        if left_choice == right_choice:
            st.warning("Choose two different raster layers for a meaningful comparison.")

    elif st.session_state["comparison_open"]:
        if active_rasters:
            left_raster = active_rasters[0]
        st.warning("Comparison needs two visible raster layers.")
    elif not active_rasters:
        st.caption("No visible raster loaded. Upload GeoTIFFs or click **🌟 Demo Data**.")
...
<div id="app">
    <div id="viewSelector">
        <div class="btn-group">
            <button id="mapButton" class="viewButton active" onclick="showMap()">🗺️ 2D Map</button>
            <button id="globeButton" class="viewButton" onclick="showGlobe()">🌍 3D Globe</button>
            <button class="viewButton" onclick="resetView()" title="Fit to Raster Bounds">🎯 Fit View</button>
        </div>
        <div style="font-size:12px; color:#5c788d; font-weight:600;">
            ↔ Drag central divider to compare layers
        </div>
    </div>
...
if (BASEMAP_STYLE.includes("Dark")) {
    tileUrl = "https://basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png?key=cb1_3unz_1_4b6a5a7d40c0b39935655c16";
